...
const App: React.FC = () => {
  ...
  async function getBarbers(barbershopId: string) {
    const url = new URL(
      `http:/localhost:3000/barbershops/${barbershopId}/barbers`,
    );
  ...
  useEffect(() => {
    ...
    if (form.customer !== undefined && isFormSent === false) {
      let id = createCustomer(form.customer);
      id.then((id) => createAppointment(id)).then((isWorked) => {
        if (isWorked) {
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        {step === 1 && <LocationForm setFormLocation={setFormLocation} />}{" "}
        {step === 2 && (
        ...
        {step === 4 && (
          <div className="twoSections">
            <DateForm
              barbershop={form.selected_location}
              barber={form.selected_barber}
              setFormBarber={setFormBarber}
              setFormTimeBarber={setFormTimeBarber}
        ...
        {step === 6 && <AppointmentConfirmed form={form} />}
      </header>
    </div>
  );
};
